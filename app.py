...
def export_data():
    ...
    return Response(
        json.dumps({'exported_at': datetime.now().isoformat(), 'applicants': data}, indent=2),
        mimetype='application/json',
        headers={'Content-Disposition': 'attachment; filename=truecare-crm-export.json'}
    )
    ...
    try:
        ...
        for a in applicants:
            ...
            existing = conn.execute(
                'SELECT id FROM applicants WHERE name = ? OR phone = ?',
                (a.get('name',''), a.get('phone',''))
            ).fetchone()
            if existing:
                conn.execute('''
                    UPDATE applicants SET
                        interview_status=?, hired_status=?, justin_notes=?,
                        chester_notes=?, start_date=?, resume_text=?,
                        interview_time=?, updated_at=?
                    WHERE id=?
                ''', (
                    a.get('interview_status','Not started'),
                    a.get('hired_status','Not started'),
                    a.get('justin_notes',''),
                    a.get('chester_notes',''),
                    a.get('start_date',''),
                    a.get('resume_text',''),
                    a.get('interview_time',''),
                    datetime.now().isoformat(),
                    existing['id']
                ))
                updated += 1
                ...
                conn.execute('''
                    INSERT INTO applicants (name, phone, email, interview_time, interview_status,
                        hired_status, justin_notes, chester_notes, start_date, resume_text,
                        source, position, state, updated_at)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                ''', (
                    a.get('name',''), a.get('phone',''), a.get('email',''),
                    a.get('interview_time',''), a.get('interview_status','Not started'),
                    a.get('hired_status','Not started'), a.get('justin_notes',''),
                    a.get('chester_notes',''), a.get('start_date',''), a.get('resume_text',''),
                    a.get('source',''), a.get('position',''), a.get('state',''),
                    datetime.now().isoformat()
                ))
                created += 1
        conn.commit()
        conn.close()
        return jsonify({'status': 'ok', 'updated': updated, 'created': created})
